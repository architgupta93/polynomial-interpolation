classdef f_gaussian < HDFunction
    ...
    methods (Access = public)
        function Obj = f_gaussian(n_in, n_out, parms);
        ...
        function y_out = evaluate(Obj, x_in)
            Obj.checkDims(x_in);
            y_out = exp(-sum(times((Obj.c_vec.^2), (minus(x_in', Obj.w_vec)).^2), 2));
        end

        function der = diff(Obj, x_in)
            Obj.checkDims(x_in);
            der = -times( times((Obj.c_vec.^2), 2*(minus(x_in', Obj.w_vec))), ...
                exp(-sum(times((Obj.c_vec.^2), (minus(x_in', Obj.w_vec)).^2), 2)) );
        end
    end
end
